...
function Sidebar() {
    ...
    const sendMail = (e) => {
        console.log("In sent email")
        e.preventDefault()
        console.log(user.email, recipient,subject,content,user,firebase.firestore.FieldValue.serverTimestamp())
        if(recipient && content !== "") {
           
          db.collection('sendMails').add({
                from: user.email,
                to: recipient,
                subject: subject,
                content: content,
